...
const placeById = new Map(places.map((row) => [row.place_id, row]));
const segmentById = new Map(segments.map((row) => [row.segment_id, row]));
const BETA_DISTRICTS = new Set(['종로', '관악']);
...
for (const place of places) {
  ...
  assert.ok(BETA_DISTRICTS.has(place.district), `${place.place_id}: 베타 생활권 밖입니다.`);
...
for (const segment of segments) {
  ...
  assert.equal(placeById.get(segment.start_place_id).district, segment.district, `${segment.segment_id}: 시작 장소와 생활권이 다릅니다.`);
  assert.equal(placeById.get(segment.end_place_id).district, segment.district, `${segment.segment_id}: 종료 장소와 생활권이 다릅니다.`);
...
for (const experience of experiences) {
  ...
  assert.equal(placeById.get(experience.place_id).district, experience.district, `${experience.experience_id}: 장소와 생활권이 다릅니다.`);
  if (experience.segment_id) assert.equal(segmentById.get(experience.segment_id).district, experience.district, `${experience.experience_id}: 산책 구간과 생활권이 다릅니다.`);
