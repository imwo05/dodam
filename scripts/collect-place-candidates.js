...
const CATEGORIES = {
  ...
  '공원': { keywords: ['공원', '어린이공원', '근린공원', '녹지', '쌈지공원', '소공원', '산', '계곡', '생태공원', '도시자연공원', '수변공원', '숲'], menu: '잠깐 쉬거나 가볍게 걷기 좋은 실제 공원·산·계곡·녹지 검색 후보', tags: '#공원 #잠깐휴식 #산책 #검수필요' },
...
const PARK_FACILITY_PATTERN = /화장실|주민센터|무더위쉼터|한파쉼터|주차장|약국|부동산|은행|금고|ATM|호텔|아파트|식당|해장|카페|커피|헤어|필라테스|학교|교회|웨딩|충전소|노래|병원|의원|마트|시장|사무소|관리사무소|안내소|대여소|경로당|회관|체육|운동장|반려|강아지|키즈/i;
...
function addCandidate({ district, category, spec, item }) {
  ...
  if (category === '공원' && (PARK_FACILITY_PATTERN.test(placeName) || PARK_FACILITY_PATTERN.test(address))) return;
  const key = `${district}:${category}`;
