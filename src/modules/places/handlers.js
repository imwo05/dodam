...
function validateCreatePlace(body, { partial = false, existing = null } = {}) {
  ...
  if (need('address')) out.address = assertRequiredString(input.address, 'address', { min: 1, max: 255 });
  ...
  if (input.description !== undefined) out.description = assertRequiredString(input.description, 'description', { min: 0, max: 2000 });
  if (input.tip !== undefined) out.tip = input.tip === null ? null : assertRequiredString(input.tip, 'tip', { min: 1, max: 500 });
